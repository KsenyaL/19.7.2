...
def test_add_new_pet_with_valid_data(name='Семен', animal_type='кот', age='4', pet_photo='images/cat1.jpg'):
...
def test_add_new_pet_with_invalid_data(name='Семен', animal_type='кот', age='20', pet_photo='images/cat1.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:   #если длина json соответствует 0
        pf.add_new_pet(auth_key, "Рыжая", "кот", '3', "images/cat2.jpg") #то добавляем инфу о новом питомце
...
иначе ругает"""
def test_successful_update_self_pet_info(name='Мурка', animal_type='кот', age='3'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_unsuccessful_update_self_pet_info(name='Снежок', animal_type='ваорп', age='-3'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    if len(my_pets['pets']) > 0:
        status, result = pf.update_pet_info(auth_key, my_pets['pets'][0]['id'], name, animal_type, age)
...
def test_simple_add_new_pet_with_valid_data(name='Семен', animal_type='кот', age='4'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
